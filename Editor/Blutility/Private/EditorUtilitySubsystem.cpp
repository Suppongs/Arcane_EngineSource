...
{
	// YSJ : Custom Edit for register custom console command to change into Landscape Mode ~
	if (!IConsoleManager::Get().FindConsoleObject(TEXT("ChangeModeLandscape")))
	{
		IConsoleManager::Get().RegisterConsoleCommand(
			TEXT("ChangeModeLandscape"),
			TEXT(""),
			FConsoleCommandWithArgsDelegate::CreateStatic(&ChangeModeLandscape),
			ECVF_Default
		);
	}
	// ~ Custom Edit for register custom console command to change into Landscape Mode
...
void UEditorUtilitySubsystem::ChangeModeLandscape(const TArray<FString>& Args)
{
