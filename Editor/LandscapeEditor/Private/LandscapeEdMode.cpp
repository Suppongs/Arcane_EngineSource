...
{
	UISettings->Modify();
	// YSJ : Custom Edit for fixing LandscapeLayer ~
	if (ALandscape* Landscape = GetLandscape())
	{
		FName FixedLandscapeLayerName(TEXT("Layer"));
		int32 FixedLandscapeLayerNum = Landscape->GetLayerIndex(FixedLandscapeLayerName);
		if (FixedLandscapeLayerNum != INDEX_NONE)
		{
			InLayerIndex = FixedLandscapeLayerNum;
		}
	}
	// ~ Custom Edit for fixing LandscapeLayer
