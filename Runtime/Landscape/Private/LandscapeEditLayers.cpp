...
// YSJ : Custom Edit for merge blueprintBrush actor to custom landscape layer ~
void ALandscape::CopyOldDataToCustomLayer()
{
	// every code in this func are same as "CopyOldDataToDefaultLayer()" except 1 line
	...
	Info->ForEachLandscapeProxy([&](ALandscapeProxy* Proxy)
	{
		// Only this line is Different from "CopyOldDataToDefaultLayer()"
		CopyOldDataToCustomLayer(Proxy);
		return true;
	});
}

void ALandscape::CopyOldDataToCustomLayer(ALandscapeProxy* InProxy)
{
	// every code in this func are same as "CopyOldDataToDefaultLayer()" except 1 line and "CustomLayer"s Name
	...
	// this line is Different from "CopyOldDataToDefaultLayer()"
	const FLandscapeLayer* CustomLayer = GetLayerConst("Layer");
	check(CustomLayer != nullptr);
	...
	{
		FLandscapeLayerComponentData* LayerData = Component->GetLayerData(CustomLayer->Guid);
		...
		if (ensure(LayerData != nullptr && LayerData->IsInitialized()))
		{
			...
			for (int32 TextureIndex = 0; TextureIndex < ComponentWeightmapTextures.Num(); ++TextureIndex)
			{
				...
				if (WeightmapTextureData != nullptr)
				{
					...
					check(WeightmapTextureData->Usage->LayerGuid == CustomLayer->Guid);
				...
				else
				{
					...
					LayerData->WeightmapData.TextureUsages[TextureIndex]->LayerGuid = CustomLayer->Guid;
...
// ~ Custom Edit for merge blueprintBrush actor to custom landscape layer
