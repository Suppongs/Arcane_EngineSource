...
module EpicGames
  ...
  module Datasmith
    ...
    def self.timeit(name, &timed)
      ...
      # don't output time for calls which don't take much time
      # to avoid slowing down everything when there are lots of events like change notifications for faces are fired
      if time_delta > 1
        Datasmith.log_info { "'#{name}' executed in #{time_delta} seconds" }
      end
    ...
    def self._load(module_dll_path)
      ...
      toolbar.add_item show_log_command
      ...
      datasmith_menu.add_item show_log_command
